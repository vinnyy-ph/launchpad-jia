        ...
        <table width="600" cellpadding="0" cellspacing="0" border="0" style="max-width: 600px; background-color: #ffffff; border-radius: 8px; padding: 24px;">
          ...
          <tr>
            <td style="padding: 24px; text-align: center;">
              ...
              <a href="${loginUrl}" style="display: inline-block; background-color: #181D27; color: #ffffff; padding: 12px 120px; border: 1px solid #999999; border-radius: 50px; text-decoration: none; font-size: 17px; font-weight: 550;">Log In</a>
            </td>
          </tr>
        </table>
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { email, name, companyName, inviterEmail, inviterName, orgId } = body;

    if (!email || !name) {
      console.error("[send-guest-invitation] Missing required data: email or name");
    ...
    // Build the magic link URL - redirect through /auth/callback for Google SSO
    ...
    const redirectPath = orgId 
      ? `/guest-portal/careers?orgId=${orgId}` 
      : '/guest-portal/careers';
    const loginUrl = `${baseUrl}/auth/callback?redirect=${encodeURIComponent(redirectPath)}`;

    const htmlContent = getGuestInvitationTemplate(
      name,
      companyName || 'White Cloak Technologies, Inc.',
      inviterEmail || '',
      inviterName || '',
      loginUrl
    );
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject: `Jia | ${companyName || 'White Cloak Technologies, Inc.'} has invited you to join`,
      html: htmlContent,
    });
