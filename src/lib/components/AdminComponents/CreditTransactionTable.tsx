...
export interface CreditTransactionTableRef {
  refresh: () => void;
}

const CreditTransactionTable = forwardRef<CreditTransactionTableRef, CreditTransactionTableProps>(
  function CreditTransactionTable({ orgId, onRefresh, frozenCredits = 0, hasPremiumPlan = false }, ref) {
    ...
    useImperativeHandle(ref, () => ({
      refresh: () => fetchTransactions(undefined, true),
    }), []);
    ...
    const fetchTransactions = useCallback(async (cursor?: string, reset = false) => {
      try {
        ...
        const params: any = { orgId, limit: 10 };
        ...
        const response = await api.get("/api/pricing-plan/admin/get-credit-transactions", { params });
    ...
    return (
      <>
        ...
        <div>
          ...
          {frozenCredits > 0 && hasPremiumPlan && (
            <div
              style={{
                marginTop: 16,
                padding: "12px 16px",
            ...
            >
              ...
              <span style={{ fontSize: 14, color: "#181D27" }}>
                <span style={{ fontWeight: 700, color: "#181D27" }}>{frozenCredits.toLocaleString()}</span> credits are currently inactive under this plan. They will become usable again if the organization switches back to a credit-based plan.
              </span>
            </div>
          )}
                            ...
                            {tx.type === "adjusted" && tx.adjustmentReason && (
                              <>
                                <i
                                  ...
                                  data-tooltip-id={`adjustment-reason-${tx._id}`}
                                  style={{
                                    fontSize: 14,
                                    color: "#98A2B3",
                                    cursor: "help",
                                ...
                                />
                                <Tooltip
                                  id={`adjustment-reason-${tx._id}`}
                                  place="top-start"
                                  className="credit-transaction-tooltip"
                                  style={{
