...
export default function DirectInterviewLinkV2(props: { formData: any, setFormData: (formData: any) => void }) {
  ...
  useEffect(() => {
      if (formData?.directInterviewLink) {
        let dynamicLink = `${window.location.origin.includes(process.env.NEXT_PUBLIC_EMPLOYER_APP_DOMAIN) ? 
          `https://${process.env.NEXT_PUBLIC_APPLICANT_APP_DOMAIN}` : window.location.origin}${formData.directInterviewLink}`;
