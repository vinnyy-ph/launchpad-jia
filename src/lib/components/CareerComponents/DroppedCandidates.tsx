...
const stickyActionsHeaderStyle: any = {
    ...tableHeaderStyle,
    position: "sticky",
    right: 0,
    zIndex: 4,
    ...
    width: "56px",
    minWidth: "56px",
    boxShadow: "-8px 0 8px -8px rgba(16, 24, 40, 0.16)",
};

const stickyActionsCellStyle: any = {
    position: "sticky",
    right: 0,
    zIndex: 3,
    background: "#FFFFFF",
...
export default function DroppedCandidates({ timelineStage, handleDroppedCandidatesOpen, handleCandidateMenuOpen, handleCandidateCVOpen, handleReconsiderCandidate }: any) {
    const { name: stage, droppedCandidates } = timelineStage;
    ...
    const handleMenuToggle = (candidate: any) => {
        const isSameCandidate = selectedCandidate?._id === candidate._id;
        setSelectedCandidate(candidate);
        setMenuOpen(isSameCandidate ? !menuOpen : true);
    };
                    ...
                    <div className="table-responsive" style={{ height: "100%", background: "#FFFFFF", borderRadius: "20px" }}>
                        <table className="table align-items-center table-flush" style={{ border: "1px solid #E9EAEB" }}>
                            <thead>
                                ...
                                <th scope="col" style={stickyActionsHeaderStyle}></th>
                                </tr>
                            </thead>
                            <tbody>
                                {filteredCandidates.length > 0 ? filteredCandidates.map((candidate: any) => (
                                    <tr key={candidate._id} style={{ cursor: "default" }}>
                                        ...
                                        <td>
                                        <CareerFit 
                                        fit={(candidate?.currentEvaluation?.matchFit) ? candidate?.currentEvaluation?.matchFit : candidate.currentStep === "CV Screening" || candidate?.stage === "Pending AI Interview" ? candidate?.cvStatus || "N/A" : candidate?.jobFit || "N/A"} 
                                        assessment={(candidate?.currentEvaluation?.matchFit) ? candidate?.currentEvaluation?.evaluationNotes : candidate.currentStep === "CV Screening" || candidate?.stage === "Pending AI Interview" ? candidate?.cvScreeningReason : extractInterviewAssessment(candidate?.summary)}
                                        ...
                                        <td>
                                            ...
                                            dangerouslySetInnerHTML={{ __html: candidate?.currentEvaluation?.evaluationNotes || "-" }}
                                            ></p>
                                        </td>
                                        ...
                                        <td
                                            style={{
                                                ...stickyActionsCellStyle,
                                                zIndex: menuOpen && candidate._id === selectedCandidate?._id ? 10 : stickyActionsCellStyle.zIndex,
                                            }}
                                        >
                                            <div className="dropdown">
                                            <button style={{ background: "none", border: "none", cursor: "pointer" }} onClick={() => handleMenuToggle(candidate)}>
                                                <span>
