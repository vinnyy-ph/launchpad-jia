...
export default function (props) {
  ...
  function addQuestion(groupId: number, newQuestion: string) {
    ...
    if (categoryIndex !== -1) {
      ...
      // Clear validation error if we now have enough questions
      if (validationErrors?.interviewQuestions && setValidationErrors) {
        const totalQuestions = updatedQuestions.reduce(
  ...
  async function generateAllQuestions() {
    try {
      ...
      const response = await api.post("/api/llm-engine", {
        systemPrompt:
        ...
        IMPORTANT: Generate all questions in ${aiInterviewLanguage || "English"}.
  ...
  async function fetchInstructionPrompt() {
    ...
    if (configData && configData.question_gen_prompt) {
        ...
        <div className="layered-card-content">
          {validationErrors?.interviewQuestions && (
            ...
            >
              ...
              ></i>
              <span style={{ fontSize: 14, color: "#EF4444", fontWeight: 400 }}>
