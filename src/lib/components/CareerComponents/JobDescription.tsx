...
export default function JobDescription({ formData, setFormData, isEditing, setIsEditing, handleCancelEdit }: { formData: any, setFormData: (formData: any) => void, isEditing: boolean, setIsEditing: (isEditing: boolean) => void, handleCancelEdit: () => void }) {
        ...
        async function fetchLinkedProject() {
            ...
            try {
                ...
                if (response.status === 200 && response.data.project) {
                    setLinkedProject(response.data.project);
                }
            } catch (error) {
    ...
    async function updateCareer() {
      const userInfoSlice = {
        ...
        try {
            const response = await api.post("/api/update-career", input);
                      ...
                      <div style={{ borderRadius: "50%", width: 30, height: 22, border: "1px solid #D5D9EB", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, backgroundColor: "#F8F9FC", color: "#181D27", fontWeight: 700 }}>
                        ...
                        {formData.preScreeningQuestions?.length > 0 ? 
                          formData.preScreeningQuestions?.map((question, index) => (
                            <div key={index} style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", marginBottom: 16, borderBottom: "1px solid #E9EAEB", paddingBottom: 16 }}>
                              <div style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
                                <span style={{ fontSize: 16, color: "#414651", fontWeight: 500, marginBottom: 8 }}>{question.questionType}</span>
                              </div>
                    ...
                    {!loadingProject && linkedProject && (
                        <div className="layered-card-outer">
                            <div className="layered-card-middle">
                                <div style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "flex-start", width: "100%", gap: 8 }}>
                                    <div style={{ width: 32, height: 32, display: "flex", justifyContent: "center", alignItems: "center", gap: 8, background: "#181D27", borderRadius: "60px" }}>
                                        <i className="la la-folder" style={{ fontSize: 20, color: "#FFFFFF"}} />
                                    </div>
                                    <span style={{ fontSize: 16, fontWeight: 700, color: "#181D27" }}>Linked Project</span>
                                </div>

                                <div className="layered-card-content" style={{ color: "#525F7F" }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 8, width: "100%" }}>
                                        <i className="la la-folder-open" style={{ fontSize: 18 }}></i>
                                        <span style={{ fontSize: 16, fontWeight: 600 }}>{linkedProject.name}</span>
                                    </div>
                                    <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                                        <i className="la la-briefcase" style={{ fontSize: 14  }}></i>
                                        <span style={{ fontSize: 14 }}>
                                            {linkedProject.careers?.length || 0} {linkedProject.careers?.length === 1 ? 'career' : 'careers'}
                                        </span>
                                    </div>
                                </div>
                            </div>
