...
import OrganizationStatus from "../AdminComponents/OrgEditing/OrganizationStatus";
import useDebounce from "../../hooks/useDebounceHook";
import Swal from "sweetalert2";
import OrganizationActionModal from "../AdminComponents/OrganizationActionModal";
import { api } from "@/lib/utils/apiClient";
import { Button } from "../ui";
import PricingPlanBadge from "../AdminComponents/PricingPlans/PricingPlanBadge";
...
export default function OrganizationsTable() {
    ...
    useEffect(() => {
        const fetchOrganizations = async () => {
            try {
                setLoading(true);
                const response = await api.get("/api/admin/get-organizations", {
                    params: {
    ...
    const handleUpdateOrganizationStatus = async (orgID: string, status: string) => {
        try {
            Swal.showLoading();
            const response = await api.post("/api/admin/update-organization", { orgID, update: { status: status } });
                                ...
                                <table className="table align-items-center table-flush">
                                    <thead>
                                        <tr>
                                            ...
                                            <th scope="col" style={tableHeaderStyle}>Plan</th>
                                        ...
                                        {loading ? <TableLoader type="careers-v2" /> : (
                                            organizations.map((organization: any) => (
                                                <tr key={organization._id}>
                                                    <td>
                                                        <div style={{ display: "flex", flexDirection: "row", gap: 8, alignItems: "center", maxWidth: "300px" }}>
                                                            <img src={organization.image} alt="logo" style={{ width: 40, height: 40, borderRadius: "50%", background: "#E0E0E0", flexShrink: 0 }} />
                                                            <div 
                                                                style={{ 
                                                                    fontSize: 14, 
                                                                    color: "#181D27", 
                                                    ...
                                                    <td>
                                                        {organization.plans && organization.plans.length > 0 ? (
                                                            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                                                                {organization.plans.map((plan: any, index: number) => (
                                                                    <div key={index} style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: 4 }}>
                                                                        <span style={{ fontSize: 14, color: "#181D27", fontWeight: 500 }}>
                                                                            {plan.name}
                                                                        </span>
                                                                        <PricingPlanBadge schema={plan.schema} />
                                                                        {index < organization.plans.length - 1 && (
                                                                            <span style={{ fontSize: 14, color: "#717680", fontWeight: 500 }}>+</span>
                                                                        )}
                                                                    </div>
                                                                ))}
                                                            </div>
                                                    ...
                                                    <td>
                                                        {(() => {
                                                            const totalBaseLimit = organization.totalBaseLimit;
                                                            const totalAdjustment = organization.totalAdjustment || 0;
                                                            const totalJobLimit = organization.jobLimit;

                                                            if (totalJobLimit === null) {
                                                                return (
                                                                    <span style={{ fontSize: 14, color: "#717680", fontWeight: 500 }}>
                                                                        Unlimited
                                                                    </span>
                                                            ...
                                                            if (totalAdjustment !== 0) {
                                                                ...
                                                                return (
                                                                    <span style={{ fontSize: 14, color: "#717680", fontWeight: 500 }}>
                                                                        {totalBaseLimit}({sign}{totalAdjustment})
                                                                    </span>
                                                                );
                                                            }
                                                            ...
                                                            {menuOpen && selectedOrganization?._id === organization._id && (
                                                                ...
                                                                >
                                                                    ...
                                                                    <div className="dropdown-item" onClick={(e) => {
                                                                        ...
                                                                        e.preventDefault();
                                                                        setMenuOpen(false);
                                                                        window.location.href = `${process.env.NEXT_PUBLIC_APP_URL}/recruiter-dashboard?orgID=${organization._id}`;
                                                                    }}>
                                                                    ...
                                                                    <div className="dropdown-item" onClick={(e) => {
                                                                        ...
                                                                        e.preventDefault();
                                                                        setMenuOpen(false);
                                                                        const url = new URL(`${process.env.NEXT_PUBLIC_APP_URL}/job-openings`);
