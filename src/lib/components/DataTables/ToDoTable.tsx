...
import { candidateActionToast, errorToast, extractInterviewAssessment, formatDateToRelativeTime, getCurrentPipelineStage, getStage } from "@/lib/Utils";
        ...
        const initialLoad = async () => {
            try {
                if (initialLoadRef.current) {
                    ...
                    setLoading(true);
                    const response = await api.get("/api/get-pending-tasks", {
                        params: {
                            orgID,
                            search: "",
    ...
    const handleCandidateAction = async (action: string) => {
        ...
        if (action === "approve-retake") {
            ...
            try {
                ...
                const currentPipelineStage = getCurrentPipelineStage(selectedCandidate.pipelineStages, {
                ...
                await api.post("/api/update-interview", {
                    ...
                    interviewTransaction: {
                        ...
                        careerId: selectedCandidate?.careerId,
                ...
                setCandidates(prevCandidates => prevCandidates.filter((candidate: any) => candidate._id !== selectedCandidate._id));
            } catch (error) {
                ...
                <tbody>
                    {isLoading ? (
                        ...
                        <tr key={candidate._id} style={{ cursor: "default" }}>
                            <td>
                                <div className="d-flex align-items-center" style={{ gap: "10px", cursor: "pointer" }} onClick={() => {
                                    ...
                                    <CandidateAvatarWithTooltip candidate={candidate} orgID={orgID || ""}>
                                        {candidate?.image ? (
                                            <AvatarImage src={candidate.image} alt="Candidate" />
                                        ) : (
                                    ...
                                    <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
                                        <a 
                                        href={`/recruiter-dashboard/careers/manage/${candidate?.id}/interview-analysis/${candidate?.interviewID}?orgID=${orgID}`} 
                                        onClick={(e) => {
                                            e.preventDefault();
                                            e.stopPropagation();
                                            setSelectedCandidate(candidate);
                            ...
                            <td><CareerFit 
                            fit={candidate?.currentEvaluation?.matchFit ? candidate?.currentEvaluation?.matchFit : candidate?.cvStatus || "N/A"} 
                            assessment={candidate?.currentEvaluation?.matchFit ? candidate?.currentEvaluation?.evaluationNotes : candidate?.cvScreeningReason}
                    ...
                    {isLoading ? (
                        ...
                        <tr key={candidate._id} style={{ cursor: "default" }}>
                           ...
                           <div className="d-flex align-items-center" style={{ gap: "10px", cursor: "pointer" }} onClick={() => {
                            ...
                            <td>{candidate.jobTitle}</td>
                            <td><CareerFit 
                            fit={candidate?.currentEvaluation?.matchFit ? `${candidate?.currentEvaluation?.updatedBy?.name?.split(" ")[0]}: ${candidate?.currentEvaluation?.matchFit}` : candidate?.jobFit || "N/A"} 
                            assessment={candidate?.currentEvaluation?.matchFit ? candidate?.currentEvaluation?.evaluationNotes : extractInterviewAssessment(candidate?.summary)}
                            evaluatorName={candidate?.currentEvaluation?.updatedBy?.name ? candidate?.currentEvaluation?.updatedBy?.name?.split(" ")[0] : "Jia"}
                            candidateDetails={candidate}
                            /></td>
                            ...
                            <button style={{ background: "none", border: "none", cursor: "pointer" }} onClick={() => {
                                router.push(`/recruiter-dashboard/careers/manage/${candidate?.id}/interview-analysis/${candidate?.interviewID}?orgID=${orgID}`);
                            }}>
