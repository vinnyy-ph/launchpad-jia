...
export default function ComposeEmailModule({
  ...
  const storedEmail = sessionStorage.getItem("viewingCandidateEmail");
  ...
  // Cleanup form when modal closes or populate when opens
  React.useEffect(() => {
    if (!isOpen) {
    ...
    } else if (isOpen && !replyData && !forwardData && !draftData) {
      // When opening fresh (not a reply, forward, or draft), set the stored email
      setFormData({
        // to: storedEmail || "",
        to: "",
        job: "",
  ...
  React.useEffect(() => {
    if (isOpen && forwardData) {
      ...
      messages.forEach((message: any, index: number) => {
        ...
        forwardedContent += `<div style="border-left: 3px solid #ccc; padding-left: 20px; margin: 20px 0; color: #666; padding-left: ">`;
  ...
  // Update form data when draftData changes (only when modal is open)
  React.useEffect(() => {
    if (isOpen && draftData) {
      ...
      setTimeout(() => {
        ...
        // Set message content in contentEditable div (includes images)
        if (messageRef.current && draftData.message) {
          messageRef.current.innerHTML = draftData.message;
        }
  ...
  // Fetch all member emails for From field when admin
  useEffect(() => {
      ...
      try {
        ...
        const response = await api.get("/api/get-careers", {
          params: {
            ...
            orgID: orgID,
            page: 1,
            limit: 100, // Get all careers
          },
        });
        const sortedCareers = (response.data.careers || []).sort((a: any, b: any) =>
          a.jobTitle.localeCompare(b.jobTitle)
        );
        setCareers(sortedCareers);
      } catch (error) {
        console.error("Error fetching careers:", error);
        errorToast("Failed to load job positions", "top-center");
      } finally {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      formDataToSend.append("message", formData.message);
      formDataToSend.append("CareerId", formData.job);
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      formDataToSend.append("messageid", draftData?.draftId || "");
      // formDataToSend.append("jobTitle", formData.job);
      // Add reply-related fields if available (only for replies, not forwards)
      if (replyData?.isReply && replyData?.threadId) {
        formDataToSend.append("threadId", replyData.threadId);
      }
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // successToast(`${fileArray.length} file(s) attached successfully`, "top-center");
  ...
  const modalOverlayStyle: React.CSSProperties = {
    ...
    right: 0,
    bottom: 0,
    backgroundColor: isMinimized ? "transparent" : "rgba(0, 0, 0, 0.3)",
    display: "flex",
    alignItems: "flex-end",
  ...
  const modalStyle: React.CSSProperties = {
    ...
    width: isMinimized ? "320px" : " ",
    maxWidth: "640px",
    // maxWidth: "100%",
    height: isMinimized ? "auto" : "672px",
    // maxHeight: "90vh",
    overflow: "hidden",
    display: "flex",
  ...
  const fromContainerStyle: React.CSSProperties = {
    display: "flex",
    alignItems: "center",
    justifyContent: "space-between",
    gap: "12px",
  ...
  const fromLeftStyle: React.CSSProperties = {
    display: "flex",
    alignItems: "center",
    gap: "10px",
  };

  const fromInfoStyle: React.CSSProperties = {
    flex: 1,
  };
  ...
  const toJobRowStyle: React.CSSProperties = {
    display: "grid",
  ...
  // Create job filter options for DropdownModal (sorted alphabetically)
  const jobFilterOptions: DropdownOption[] = careers
    .slice()
    ...
    <>
      <style>{`
        ...
        .from-dropdown-button:hover:not(:disabled) {
          ...
          .compose-email-modal button:focus {
      ...
      <div
        ...
        onClick={isMinimized ? undefined : () => setIsMinimized(true)}
      ...
      >
        <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
          ...
          >
            ...
            <div style={windowControlsStyle} onClick={(e) => e.stopPropagation()}>
              <button
              ...
              >
                <i className="la la-minus" style={{ fontSize: "14px" }}></i>
              </button>
              <button
                style={windowControlButtonStyle}
                ...
                onMouseLeave={(e) => e.currentTarget.style.backgroundColor = "transparent"}
              >
                <i className="la la-expand" style={{ fontSize: "14px" }}></i>
              </button>
              <button
                style={windowControlButtonStyle}
                onClick={onClose}
          ...
          {/* Content */}
          <div style={{ ...contentStyle, display: isMinimized ? 'none' : 'block' }}>
            {/* From Field */}
            <div style={{ ...formSectionStyle, marginBottom: "16px" }}>
              <div style={fromContainerStyle}>
                <div style={fromLeftStyle}>
                  ...
                  {isAdmin && fromEmailOptions.length > 0 ? (
                    <DropdownModal
                      ...
                      style={{ flex: 1, minWidth: "210px", maxWidth: "100%" }}
                      showImg={true}
                      menuStyle={{ minWidth: "370px" }}
            ...
            {/* To and Job Fields in One Row */}
            <div style={toJobRowStyle}>
              <div>
              ...
              <div>
                ...
                <DropdownModal
                  ...
                  onSelect={(value) => handleInputChange("job", value)}
                  placeholder={isLoadingCareers ? "Loading jobs..." : "Select a job position"}
                  disabled={isLoadingCareers}
                  buttonClassName="from-dropdown-button"
                  containerClassName="from-dropdown-container"
                  style={{ flex: 1, minWidth: "260px", maxWidth: "100%", width: "300px" }}
            ...
            <div style={formSectionStyle}>
              ...
              <div style={{
                ...
                {attachedFiles.length > 0 && (
                  <div style={{
                    ...
                    // borderTop: "1px solidhsl(220, 13.00%, 91.00%)",
                    marginTop: "10px"
                  }}>
              ...
              <div style={toolbarStyle} className="compose-email-toolbar">
                <select
                  style={{ ...toolbarSelectStyle, fontSize: "14px" }}
                  value={fontSize}
                ...
                >
                  ...
                  <span style={{ fontSize: "12px", marginLeft: "1px" }}>Insert Token</span>
                </button>
              </div>
          ...
          {/* Footer */}
          <div style={{ ...footerStyle, display: isMinimized ? 'none' : 'flex' }} className="compose-email-footer">
            ...
            <div style={footerRightStyle}>
              ...
              <button
                ...
                disabled={isSending}
              // onMouseEnter={(e) => !isSending && (e.currentTarget.style.backgroundColor = "#1D4ED8")}
              // onMouseLeave={(e) => !isSending && (e.currentTarget.style.backgroundColor = "#2563EB")}
              >
                {isSending ? (
                  <>
