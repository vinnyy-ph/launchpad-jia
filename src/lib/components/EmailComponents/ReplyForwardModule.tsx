...
interface ReplyForwardModuleProps {
    isOpen: boolean;
    onClose: () => void;
    replyData?: {
        to: string;
...
export default function ReplyForwardModule({
    isOpen,
    onClose,
    replyData,
    forwardData,
    ...
    const handleSaveDraft = async () => {
        ...
        try {
            ...
            formDataToSend.append("isReply", user?.email );
    ...
    return (
      ...
      `}</style>
            ...
            // className="reply-forward-email-container"
            >
                                    ...
                                    <>
                                        ...
                                        >
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    setShowReplyDropdown(false);
                                                    // Capture current form data before opening modal
                                                    const currentMessage = messageRef.current?.innerHTML || formData.message;
                                                    // Update replyData/forwardData with current form state
                                                    if (replyData) {
                                                        const updatedReplyData = {
                                                            ...replyData,
                                                            to: formData.to || replyData.to,
                                                            subject: formData.subject || replyData.subject,
                                                            currentMessage: currentMessage,
                                                            attachedFiles: attachedFiles,
                                                        };
                                                        if (onPopupModal) {
                                                            onPopupModal({ replyData: updatedReplyData });
                                                        }
                                                    } else if (forwardData) {
                                                        const updatedForwardData = {
                                                            ...forwardData,
                                                            subject: formData.subject || forwardData.subject,
                                                            currentMessage: currentMessage,
                                                            attachedFiles: attachedFiles,
                                                        };
                                                        if (onPopupModal) {
                                                            onPopupModal({ forwardData: updatedForwardData });
                                                        }
                                                    }
                                                    onClose(); // Close ReplyForwardModule
                                                }}
                                                style={{
                                                    width: "100%",
                                                    padding: "10px 16px",
