    ...
    const columns: KanbanColumn[] = (stage.substages || []).map((substage: any) => {
      ...
      const cards: CandidateCard[] = matchingInterviews.map((interview) => {
        ...
        return {
          ...
          // IMPORTANT: many guest-portal APIs (CV analysis, comments) use `interviewID` (not Mongo `_id`)
          // Recruiter dashboard comment threads also key by `interviewID`, so prefer that field when present.
