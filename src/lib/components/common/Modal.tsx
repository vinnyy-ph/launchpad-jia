...
import { getCurrentPipelineStage, getStage } from "@/lib/Utils";
...
export default function ({ modalType, setModalType }) {
  ...
  async function manageInterview() {
    ...
    if (modalType == modal[6]) {
      let currentStage;
      if (interviewData?.currentStep && interviewData?.status && interviewData?.pipelineStages) {
        currentStage = getCurrentPipelineStage(interviewData.pipelineStages, {
          status: interviewData.status,
          currentStep: interviewData.currentStep,
        });
      }
      data = {
        interviewData,
        ...
        interviewTransaction: {
          ...
          careerId: interviewData?.careerID,
  ...
  return (
    <div className={styles.modal}>
      ...
      {modalType == modal[3] && (
        <div className={`${styles.modalContainer} ${styles[modalType]}`}>
          <Image alt="loading" src="/gifs/loading.gif" />
