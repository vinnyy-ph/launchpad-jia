...
export default function () {
  ...
  const buttonStatus = [
    ...
    },
    {
      ...
      step: [applicationStep[1], applicationStep[0]],
      handleClick: function (interview) {
        setLoading(true);
  ...
  async function manageInterview(data) {
    ...
    await api.post("/api/whitecloak/manage-application", data)
      .then(() => {
        location.reload();
      })
      .catch((err) => {
  ...
  function processButtonState(interview): any {
    ...
    return buttonStatus.find((btn) => {
      if (btn.step) {
        return btn.step.includes(interview.currentStep) && btn.status === interview.status
      }
      return btn.status === interview.status
    });
  }
  ...
  function processState(interview, step, isImage: boolean = false) {
    const pipelineStageSteps = interview?.pipelineStages?.map((stage) => stage.name);
    const stepIndex = pipelineStageSteps.indexOf(step);
    let currentStepIndex = pipelineStageSteps.indexOf(interview.currentStep);

    if (interview.currentStep == "Applied") {
      return isImage && step == pipelineStageSteps[0]
        ? stepStatus[2]
        : stepStatus[1];
    }
    ...
    // old data
    if (interview.status == applicationPhase[7] && !["Human Interview", "Job Interview"].includes(interview.currentStep)) {
      currentStepIndex = 0;
    }
    ...
    if (currentStepIndex == stepIndex) {
      const substageIndex = interview?.pipelineStages?.[stepIndex]?.substages.findIndex((substage) => substage.status === interview.status);
      if (
        substageIndex === interview?.pipelineStages?.[stepIndex]?.substages.length - 1
      ) {
        return stepStatus[0];
      }
      return stepStatus[2];
    ...
    const lastSubstages = [];
      for (const stage of interview.pipelineStages) {
        const lastSubstage = stage.substages?.[stage.substages?.length - 1];
        if (lastSubstage) {
          lastSubstages.push(lastSubstage.status);
        }
      }
    if (
      currentStepIndex + 1 == stepIndex &&
      isImage &&
      !lastSubstages.includes(interview.status)
    )
      return stepStatus[2];
  ...
  useEffect(() => {
    ...
    async function fetchInterviews() {
      await api.post("/api/whitecloak/fetch-interviews")
        .then(async (res) => {
              ...
              {(interview.applicationStatus == interviewStatus[0] ||
                ...
                <>
                  <div className={styles.applicationStepContainer}>
                    {interview.pipelineStages.map((step: {name: string, substages: {name: string, currentStep: string, status: string}[]}, index) => (
                      <div className={styles.stepContainer} key={index}>
                        <div className={`webView ${styles.indicator}`}>
                          <img
                            alt="step"
                            src={`/icons/${processState(interview, step.name, true)
                              .toLowerCase()
                              .replace(" ", "-")}.svg`}
                          />
