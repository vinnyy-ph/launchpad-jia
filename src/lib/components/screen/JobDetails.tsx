...
export default function ({ params }) {
  ...
  const fetchJobDetails = async (jobId) => {
    ...
    try {
      const response = await api.post("/api/job-details", { jobId });
  ...
  const fetchUserInterviews = async () => {
    ...
    try {
      const response = await api.post("/api/whitecloak/fetch-interviews");
  ...
  async function applyJob() {
    ...
    await api.post("/api/whitecloak/apply-job", { selectedCareer })
      .then((res) => {
        if (res.data.error) {
          ...
          <div className={styles.jobDetailsContainer}>
            ...
            {!interviews.some(
              ...
              <div className={styles.buttonContainer}>
                <button id="apply-now-button" className={styles.apply} onClick={handleApply}>
              ...
              <div className={styles.appliedDetails}>
                <div>
                  <img alt="check" src="/icons/checkV5.svg" />
                  <span>
                    Applied{" "}
                    {processDate(
                      interviews.find(
